...
public class DoodleView extends View
{
   ...
   private final Paint paintScreen; // used to display bitmap onto screen
   ...
   public DoodleView(Context context, AttributeSet attrs)
   {
      ...
      paintLine = new Paint();
//      paintLine.setAntiAlias(false); // smooth edges of drawn line
      paintLine.setColor(Color.BLUE); // default color is black
      paintLine.setStyle(Paint.Style.STROKE); // solid line
      paintLine.setStrokeWidth(15); // set the default line width
...
//         int pointerIndex = event.findPointerIndex(pointerID);
   ...
   private void touchEnded(int lineID) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Set<Integer> pointerIDs = pathMap.keySet();
        for (Integer pointerID : pointerIDs)
            canvas.drawPath(pathMap.get(pointerID), paintLine); // draw line
